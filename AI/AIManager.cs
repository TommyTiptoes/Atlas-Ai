...
namespace MinimalApp.AI
{
    public static class AIManager
    {
        ...
        static AIManager()
        {
            ...
            providers[AIProviderType.WindowsAI] = new WindowsAIProvider();
            
            LoadSettings();
            
            // Auto-check Windows AI availability in background
            _ = Task.Run(async () =>
            {
                try
                {
                    var windowsAI = providers[AIProviderType.WindowsAI];
                    await windowsAI.ConfigureAsync(new Dictionary<string, string>());
                    if (windowsAI.IsConfigured)
                    {
                        System.Diagnostics.Debug.WriteLine("[AIManager] Windows AI (Phi Silica) is available!");
                    }
                }
                catch { }
            });
