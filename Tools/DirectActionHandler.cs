...
namespace MinimalApp.Tools
{
    ...
    public static class DirectActionHandler
    {
        ...
        public static async Task<string?> TryHandleAsync(string input)
        {
            ...
            // === WINDOWS COMMANDS ===
            var windowsCmdResult = await Agent.WindowsCommands.TryHandleAsync(text);
            if (windowsCmdResult != null) return windowsCmdResult;
            ...
            // === SCREENSHOT NOTIFICATION CONTROL - CHECK FIRST! ===
            // Must be checked BEFORE kill intent since "disable" triggers kill intent
            if (lower.Contains("screenshot") && (lower.Contains("disable") || lower.Contains("turn off") || 
                lower.Contains("stop") || lower.Contains("notification")))
            {
            ...
            if ((lower.Contains("screenshot") || lower.Contains("screen capture") || lower.Contains("snip")) &&
                !lower.Contains("notification") && !lower.Contains("disable") && !lower.Contains("enable") &&
                !lower.Contains("turn off") && !lower.Contains("turn on") && !lower.Contains("stop"))
            {
