...
- ""tell me a joke"" -> intent: unknown, confidence: 0.2";
        ...
        private static ParsedIntent FallbackParse(string message)
        {
            ...
            // Disable screenshot notifications - return null to let ToolExecutor handle it
            if (ContainsAny(msg, "disable screenshot", "turn off screenshot", "stop screenshot notification"))
            {
                result.Intent = IntentType.Unknown; // Let ToolExecutor handle this
                return result;
            }
