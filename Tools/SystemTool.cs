    ...
    public static class SystemTool
    {
        ...
        public static async Task<string> OpenAppAsync(string appName)
        {
            try
            {
                ...
                catch
                {
                    ...
                    {
                        return $"❌ Could not find {appName}. Try the exact app name.";
                    }
                }
            }
            catch (Exception ex)
            {
                return $"❌ Failed to open {appName}: {ex.Message}";
            }
        }
        ...
        public static Task<string> SortFilesByTypeAsync(string path)
        {
            try
            {
                ...
                if (moved == 0)
                    return Task.FromResult($"📁 No files to organize in {Path.GetFileName(path)} - folder is already tidy or contains no matching file types!");
                ...
                var result = $"✅ Done! Organized {moved} files in {Path.GetFileName(path)}:\n{details}\n\n💡 Say 'undo' to reverse this.";
                
                if (errors.Count > 0 && errors.Count <= 5)
                    result += $"\n\n⚠️ {errors.Count} files couldn't be moved (in use or protected)";
                
                return Task.FromResult(result);
            }
            catch (Exception ex)
            {
                return Task.FromResult($"❌ Error sorting files: {ex.Message}");
            }
        }
        ...
        public static Task<string> ConsolidateFilesAsync(string path)
        {
            try
            {
                ...
                var result = $"✅ Done! Moved {filesMoved} files to {Path.GetFileName(path)}";
                if (foldersDeleted > 0)
                    result += $", deleted {foldersDeleted} empty folders";
                    
                if (errors.Count > 0 && errors.Count <= 3)
                    result += $"\n⚠️ {errors.Count} files couldn't be moved";
